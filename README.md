<h1 align="left">Su Hong Yoon</h1>
<p align="left">
  Geoinformatics Undergraduate @ University of Seoul<br>
  Spatial Data Analysis · Hydrological Modeling · Systems Development
</p>

---
 
## About Me
 
I am an undergraduate student majoring in Geoinformatics at the University of Seoul.
 
My current focus is on my **graduation thesis**, which investigates the spatial characteristics of river flow response to precipitation — with particular emphasis on Curve Number (CN) and Lag-Time analysis using GIS and statistical methods.
 
Alongside my research, I am developing practical software projects including a remote desktop tool and a game built in Unity.
 
<details>
<summary>日本語</summary>
<br>
ソウル市立大学 空間情報工学科に在学中の学部生です。<br>
現在は卒業論文として、降雨に対する河川流出応答の空間的特性を研究しており、GISおよび統計手法を用いてCurve Number（CN）とLag-Timeの分析を行っています。<br>
研究の傍ら、リモートデスクトップツールの開発やUnityを使ったゲーム制作にも取り組んでいます。
</details>
<details>
<summary>한국어</summary>
<br>
서울시립대학교 공간정보공학과에 재학 중인 학부생입니다.<br>
현재 강수에 대한 하천 유출 응답의 공간적 특성을 분석하는 졸업논문을 작성 중이며, GIS 및 통계 기법을 활용하여 CN과 Lag-Time을 중심으로 연구를 진행하고 있습니다.<br>
연구 외에도 원격 데스크탑 툴 개발과 Unity 게임 제작 프로젝트에 참여하고 있습니다.
</details>

---
 
## Research & Activities
 
- **Graduation Thesis** — Spatial analysis of precipitation–runoff relationships (CN & Lag-Time)
- **Paper in preparation** — Based on thesis research
- Member, Korean Society of Surveying, Geodesy, Photogrammetry and Cartography
- Member, Korean Society of Geographic Information Studies
- Presenter, Korean Society of Geographic Information Studies Conference
- Participant (upcoming), IAG'i International Conference on Geomorphology @ Tokyo Institute of Science (Ookayama Campus)
---
 
## Current Projects
 
| Project | Description | Stack |
|---|---|---|
| Graduation Research | CN & LagTime spatial analysis, statistical modeling | MATLAB, ArcGIS, R |
| RemoteGuard-CPP | Remote desktop monitoring & control tool | C++ |
| Helicopter Firefighting Simulation | 3D firefighting simulation game | C#, Unity |
 
---
 
## Tech Stack
 
<div align="left">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/cplusplus/cplusplus-original.svg" height="36" />
  &nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/csharp/csharp-original.svg" height="36" />
  &nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" height="36" />
  &nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/matlab/matlab-original.svg" height="36" />
  &nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/r/r-original.svg" height="36" />
  &nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/postgresql/postgresql-original.svg" height="36" />
</div>
<br>
**Tools & Platforms** &nbsp; ArcGIS · HEC-HMS · Unity · Git
 
---
 
## Note
 
Repositories are being continuously updated with both research code and personal projects.
Some projects are under active development and may change over time.
